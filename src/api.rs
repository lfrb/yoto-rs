...
use reqwest::{blocking::Client, header, header::HeaderMap, StatusCode};
...
impl YotoClient {
    pub fn new(client_id: &str, token: Option<Token>) -> YotoClient {
        YotoClient {
    ...
    pub fn get_object<T: DeserializeOwned>(&self, endpoint: impl AsRef<str>, params: Option<&HashMap<&str, &str>>) -> T {
        let token = self.ensure_token().unwrap();
    ...
    pub fn delete_object(&self, endpoint: impl AsRef<str>) {
        let token = self.ensure_token().unwrap();
        let url = format!("{}{}", BASE_URL, endpoint.as_ref());
        self.client.delete(url).bearer_auth(&token.access_token).send().unwrap();
    }
    ...
    pub fn get_cards(&self) -> Vec<Card> {
        self.get_objects::<CardList>("/content/mine")
            .cards
    }

    pub fn get_card(&self, id: &str, playable: bool) -> Card {
        ...
        self.get_object::<ContentResponse>(endpoint, Some(&params)).card
    }
    ...
    pub fn get_family_images(&self) -> Vec<Image> {
        self.get_objects::<ImageList>("/media/family/images")
            .images
    }
