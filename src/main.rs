...
fn store_token(entry: &Entry, client: &api::YotoClient) {
    entry
...
fn main() {
    let m = App::new("yoto-cli")
        ...
        .subcommand(
            App::new("card").subcommand(App::new("info").arg(Arg::with_name("id").index(1))),
        )
    ...
    let mut client = api::YotoClient::new(CLIENT_ID, token);
        ...
        Some(("card", command)) => match command.subcommand() {
            Some(("info", arg)) => {
                if let Some(id) = arg.value_of("id") {
                    println!("Card {}:", id);
                    let card = client.get_card(id, true);
                    println!("{:?}", card);
                }
            }
            _ => {
